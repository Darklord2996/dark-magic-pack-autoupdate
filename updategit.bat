...
IF ERRORLEVEL 1 (
    IF EXIST PortableGit (
        ...
        echo Extracting Git...
        powershell -Command "Start-Process 'PortableGit48.exe' -ArgumentList '-y', '-gm2' -NoNewWindow -Wait"
        @REM set PATH="%cd%\PortableGit\cmd";%PATH%
    )
    ...
    IF EXIST .git (
        ...
        "%cd%\PortableGit\cmd\git" fetch origin -v --progress
        "%cd%\PortableGit\cmd\git" checkout -f master
    )
    echo Updating
    "%cd%\PortableGit\cmd\git" fetch -v --progress
